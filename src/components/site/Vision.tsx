...
export function Vision() {
  const [visionRef, visionY] = useLocalParallax([-40, 42]);

  return (
    <section
      ref={visionRef}
      id="vision"
      className="relative bg-ink text-background py-32 md:py-48 overflow-hidden"
    >
      <div className="absolute inset-0 opacity-30">
        <motion.img
          style={{ y: visionY, scale: 1.08 }}
          src={visionImg}
      ...
      <div className="relative mx-auto max-w-[1600px] px-6 md:px-10">
        ...
        <h2 className="font-display text-display-xl leading-[0.96] reveal max-w-[14ch]">
        ...
        <div className="mt-20 grid grid-cols-1 md:grid-cols-3 gap-10 max-w-5xl">
          <p className="text-lg leading-[1.75] text-background/70 reveal">
            A business is only meaningful if it serves a generation longer than the one that built
            it.
          </p>
          <div className="hidden md:block" />
          <p className="text-lg leading-[1.75] text-background/70 reveal md:text-right">
